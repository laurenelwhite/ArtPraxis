...
export const STAGE_COMPLETION_TARGET: Record<StageId, string> = {
  ...
  "value-study": "~15% — light planning map: 3–4 soft value groups, mostly white paper",
...
export const STAGE_IMAGE_SPECS: Record<StageId, StageImageSpec> = {
  ...
  "value-study": {
    label: "Value Map",
    intent:
      "Light instructional tonal map (~15%): 3–4 broad value groups with mostly white paper — not a grayscale painting.",
    directive:
      "Create a light instructional value study of the exact reference composition. This is a planning diagram, not a finished monochrome painting. Preserve large areas of white paper and very light value. Simplify the scene into three or four broad tonal groups. Keep the sky almost white, the sunlit flowers light, and the foliage primarily light-to-middle gray. Use dark gray only for a few small structural accents and the deepest foliage gaps. Preserve the luminous watercolor feeling. Do not create a photographic grayscale conversion, dense graphite rendering, dramatic contrast, black areas, detailed textures, or a fully shaded finished image. Roughly 45–60% of the image should remain white or nearly white; darkest-value coverage should stay below about 10–15%; no true black.",
  },
  "first-wash": {
    label: "First Wash",
    intent:
...
export function buildMasterPrompt(tutorial: Tutorial, medium: Medium): string {
  ...
  const subjectLabel = neutralizeSubjectTitle(tutorial.title);
  const subjectContext = [
    tutorial.title,
    tutorial.overview,
    tutorial.composition.focalPoint,
    ...tutorial.composition.majorShapes,
    ...tutorial.creativeChoices,
  ].join(" ");

  const parts = [
    `Edit the provided reference photograph into one finished, fully resolved master ${medium} painting on ${surface}.`,
    `Lesson subject: ${subjectLabel}.`,
  ...
  if (subjectLooksLikeSculpture(subjectContext) || subjectLooksLikeSculpture(subjectLabel)) {
    parts.push(sculptureContextSentence());
  }
...
function stageExtraGuard(stageId: StageId): string {
  switch (stageId) {
    case "value-study":
      return [
        "SCOPE LIMIT: light planning diagram only.",
        "NEGATIVE CONSTRAINTS (must avoid): photographic grayscale conversion; dense graphite shading; dramatic contrast; true black or near-black fills; dark or mid-gray sky; shading inside every object; detailed petal/leaf/texture rendering; a fully shaded finished monochrome painting; medium-dark overall tonality.",
        "REQUIRED: keep sky almost white; keep sunlit flowers very light; group distant trees/garden into soft middle values; darkest gray only for sparse structural accents and deep foliage gaps; 3–4 broad value groups; most of the paper remains white or nearly white.",
      ].join(" ");
    case "first-wash":
